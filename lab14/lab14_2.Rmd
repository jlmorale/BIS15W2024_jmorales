...
ui <- fluidPage(titlePanel("Log10 Homerange by Taxon"),
...
ui <- fluidPage(
    ...
    sliderInput("pointsize", "Select the Point Size", min = 0.5, max = 3, value = 1, step = 0.5), #allows that control of the size points
...
ui <- dashboardPage(
  ...
  dashboardSidebar(disable = T), #turn off side bar
  dashboardBody(
     selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), 
              selected = "bill_length_mm"),
...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidRow(
      box(width = 3,
          selectInput("x", "Select Population", choices = unique(wolves$pop)), hr()),#closes 1st box, give some help w/ multiple observations
      
      box(width = 7,
          plotOutput("plot", width = "600px", height = "400px"))#closes 2nd box
  )#closes fluidRow
