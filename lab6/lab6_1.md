...
```r
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") %>% clean_names() #clean names only clean the names in the variables or the column names 
```
...
```r
fish %>% #loading fish 
  mutate(length_mm = length*10) %>% # we are making a new variable, in the fish data it is just called length, the mutate function will make a new varibale in millimeter 
...
```r
fish %>% # use shift, command, and M to get pipe 
  mutate(length_half = length/2) %>% #we are making a new variable length with the 
...
```r
mammals %>%
  mutate_all(tolower) #taking all the data frame and making it all lower case
```
...
```r
mammals %>% 
  mutate(across(c("order", "family"), tolower)) #mutate across only the family and order data will turn to ower case  
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #we are making a new column, if it looks in to the newborn and find the value -999.00 we are going to change that in to a NA, if its not that value leave it as newborn value  
  arrange(newborn)
```
