...
```r
life_history %>% 
  count(order, sort = T) %>% # sort is the same as arrange
...
```r
#categorical variable, this code is much simpler 
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm = T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ #reorder goes within the aesthetic function and orders the x-axis 
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
```r
#since these are two continous data we use a scatterplot
elephants %>% 
...
```r
#rel() is for the font size of the title relative to the x and y axis, hjust() is for the placement of the title
```
