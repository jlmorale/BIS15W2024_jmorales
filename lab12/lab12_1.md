...
```r
homerange %>% 
  ggplot(aes(x=class, y=log10.mass, fill = class))+ #fill adds the color 
  geom_boxplot()
```
...
```r
homerange %>% 
  ggplot(aes(x=class, y=log10.mass, group = taxon))+ #group will group the taxon 
  geom_boxplot()
```
...
```r
homerange %>% 
  ggplot(aes(x=class, y=log10.mass, fill = taxon))+ #fill will add a color to the grouped taxon 
  geom_boxplot()
```
...
```r
#example of a hard question on midterm
deserts %>% 
  filter(species_id=="DM" | species_id=="DS") %>% 
  mutate(year=as.factor(year)) %>% #if you leave year as a numeric R is going to think you are going to use that years in math 
...
deserts %>% 
  ...
  labs(title = "Number of Samples for Species for DM and DS",
       x="year",
       y="n")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "royalblue", bins=10)+ #color is the outline and fill is the inside part of the box
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deepskyblue4", alpha  =0.4, color = "black")+ #alpha is the transparency, density is a probability plot very similar to histogram
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(color = "black", fill="rosybrown", bins = 20)+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+ #only need to specify an x
  geom_density(color = "black", fill="rosybrown")+
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y=after_stat(density)), fill = "rosybrown", alpha = 0.5, color = "black")+
...
![](lab12_1_files/figure-html/unnamed-chunk-24-1.png)<!-- -->
