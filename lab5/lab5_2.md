...
```r
fish %>% # use shift, command, m to get this, this is a pipe  
...
```r
mammals %>% 
  select(family, genus, species, gestation) %>% #you do not have to add the name of the data frame because we are saying that we will pull from the data frame mammals
  filter(gestation>=15)
```
...
```r
fish %>% #using fish data
  select(lakeid, scalelength) %>% #selecting the lakeid and scalength 
  arrange(scalelength) #arranging by scalelength
```
...
```r
fish %>% 
  select(lakeid, radii_length_mm, scalelength, fish_id) %>% 
  filter(fish_id==300) %>% 
...
```
## # A tibble: 4 × 4
##   lakeid radii_length_mm scalelength fish_id
##   <chr>            <dbl>       <dbl>   <dbl>
## 1 AL                1.23        3.02     300
## 2 AL                2.14        3.02     300
## 3 AL                2.67        3.02     300
