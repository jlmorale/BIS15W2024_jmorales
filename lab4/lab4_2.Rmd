...
```{r}
filter(fish, lakeid == "AL") #with filter the quotes are important
```
...
```{r}
filter(fish, length >= 350) #no quotes for the number
```
...
```{r}
not_AL <- filter(fish, lakeid != "AL") #remember ! means 'not' 
```
...
```{r}
filter(fish, length %in% c(167, 175)) #pulling all the fish with the length of 167 and 175, %in% means within 
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
filter(fish, between(scalelength, 2.5, 2.55)) #use between(name of column name, num., num.)
```
...
tidyverse is a big library, dplyr can be used with 
filter can be used with in the rows, or across the data frame
