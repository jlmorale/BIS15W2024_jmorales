...
library("tidyverse") #remember do not re install the package just use library fxn 
```
...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #readr is part of tidyverse 
```
...
every variable is in its own column, every observation is its own row, each value has its own box 
...
really helpful with very large data or data of interest 
...
```{r}
select(fish, "lakeid", "scalelength") #select will alow to just get data from those variables. select(data frame, "variable", "variable)
```
...
```{r}
select(fish, starts_with("radii")) #can also use ends_with()
```
...
```{r}
select_if(fish, ~!is.numeric(.)) #selecting everything that is not numeric, ~ means look at all data, ! means do not select, if the ! was not present the code will give you everything that is numeric
```
...
```{r}
mammals_new <- rename(mammmal_life, genus="Genus", wean_mass="wean.mass", max_life="max..life", litter_size="litter.size", litters_per_year="litters.year")
```
...
```{r}
select_if(mammals_new, is.character) #different way to use: select_if(mammals_new, ~is.character(.))
```
...
```{r}
select_all(mammmal_life, tolower)
```
