...
#sees all the files that have that .csv with in the path 
```
...
```r
#we will look into the list and make it into one .csv
...
```r
#look at the third element on the list 
spider_list[[3]]
#> # A tibble: 1 × 9
...
```r
names(spider_list[[6]]) #have to use the double bracket 
#> [1] "Accession" "Family"    "Genus"     "Country"   "State"     "County"   
#> [7] "Locality"  "Date"      "Collector"
glimpse(spider_list[[6]])
#> Rows: 3
#> Columns: 9
#> $ Accession <dbl> 9038519, 9038520, 9038784
#> $ Family    <chr> "Telemidae", "Telemidae", "Telemidae"
#> $ Genus     <chr> "Usofila", "Usofila", "Usofila"
#> $ Country   <chr> "USA", "USA", "USA"
#> $ State     <chr> "California", "California", "California"
#> $ County    <chr> "Del Norte", "Del Norte", "Del Norte"
#> $ Locality  <chr> "18mS Klamath", "18mS Klamath", "Jedediah Smith Redwoods Sta…
#> $ Date      <chr> "19/9/1953", "19/9/1953", "20/4/1994"
#> $ Collector <chr> "Gilbert-Schuster", "Gilbert-Schuster", "LJ Boutin"
```
...
```r
#we just want the names so we are adding a space between the county name and .csv
...
#> # ℹ 260 more rows
#using bind_rows() we are going to mush all of the data togther into one big data set 
```
...
#customer_ID is the unique handle in both data sets 
#inner_join is only going to join the data that match each other 
```
...
```r
#will join everything together from both left and right data 
...
```r
#coverts the dates in a way that R can understand 
