...
## Data Frames
During lab 2, you learned how to work with vectors and data matrices. Today we will organize the vectors into a new type of data structure called a **data frame**. The data frame is the most common way to organize data within R. You can think of a data frame as similar to a spreadsheet. A data frame can store data of many different classes.  can be numerics and characters all mixed.
...
```r
Sex <- c("male", "female", "male") #different class of data 
...
```r
dim(hbirds) #column = variable, read as 3*3  
```
...
```r
str(hbirds) #each variable is in a row
```
...
Let's use lowercase names when we create the data frame. We just changed to lowercase here, but we could use any names we wish. all lowercase and no special characters, b/c R can get confused  
...
```r
mean(hsprings$temp_C) #much cleaner 
```
...
```r
write.csv(hsprings, "hspring.csv", row.names = FALSE)
```
