...
#the data presented in this way shows that there is no x available 
```
...
```r
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") #we are separting the patient column into patient name and sex and they are separted by the underscore 
```
...
```r
heartrate3 %>% 
  unite(patient_sex, "patient", "sex", sep=" ") #unite reunites the data 
```
...
```r
#pivot longer is to turn it back to the previous data set allowing for R to understand the data better 
beachbugs_wide %>% 
  pivot_longer(-year,
               names_to = "site",
               values_to = "buglevels")
