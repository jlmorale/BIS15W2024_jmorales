...
The data is untidy because the column names are observations when in reality they are not.
...
```r
#this table is a lot easier to read for R
```
...
billboard2 <- 
  billboard %>% 
  pivot_longer(wk1:wk76, # a range of columns, instead of writing each week from 1 to 76 this is going to be much easier
               names_to = "week",
               values_to = "rank", 
...
billboard3 <- 
  billboard %>% 
  pivot_longer(-c(artist, track, date.entered), #specific columns to stay fixed, move everything except these
               names_to = "week",
               values_to = "rank",
               values_drop_na = TRUE
               )
...
The data is not tidy because each day was entered as an observation. 
...
qpcr_untidy %>% 
  pivot_longer(
    ...
    names_sep="_",# this pulls apart the name into individual names
